//! Bakes the Glance icon and version information into both executables, so
//! Explorer, the Start menu, the taskbar and Task Manager show Glance rather
//! than a generic program called `glance.exe`.
//!
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=../glance-ui/src/icon.rs");
    ...
    let out = PathBuf::from(env::var("OUT_DIR").expect("cargo sets OUT_DIR")).join("glance.res");
    fs::write(&out, res).expect("write glance.res");
