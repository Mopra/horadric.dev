//! `glance reload`: the shortest way from a new build to using it.
//!
//! Run from the new build (`target\release\glance.exe reload`), it asks the
//! running Glance to hand over. That one waits until no session is mid
//! turn, saves, starts this binary as `glance swap`, and quits. `swap` waits
...
use glance_hooks::listener::Reload;
use glance_hooks::{client, COMMAND_HEADER, RELOAD_PATH};
...
/// `glance reload [--now]`, from the command line.
pub fn request(args: &[String]) -> Result<(), String> {
    let now = match args {
        [] => false,
        [flag] if flag == "--now" => true,
        _ => return Err("usage: glance reload [--now]".into()),
    };
    let exe = std::env::current_exe().map_err(|e| e.to_string())?;
    if !glance_hooks::dev() {
        let glancew = exe.with_file_name("glancew.exe");
        if !glancew.is_file() {
            return Err(format!("{} not found (build it first)", glancew.display()));
        }
    }
    let request = Reload {
    ...
    match client::post(
        ...
        Ok(200) => {
            if now {
                println!("Glance is reloading into {}.", exe.display());
            } else {
                println!(
                    "Glance reloads into {} as soon as no session is working.",
                    exe.display()
                );
            }
            println!(
        ...
        Ok(403 | 404) => Err(format!(
            "the Glance on port {port} is too old to reload. Install this build once by hand."
        )),
        Ok(status) => Err(format!("Glance answered {status}")),
        Err(_) => Err(format!("Glance is not running (nothing on port {port})")),
    }
}

/// `glance swap --pid N`, started by the app that is handing over.
pub fn swap(args: &[String]) -> Result<(), String> {
    let pid = match args {
        [flag, pid] if flag == "--pid" => pid.parse::<u32>().map_err(|e| e.to_string())?,
        _ => return Err("usage: glance swap --pid N".into()),
    };
...
fn swap_inner(pid: u32, log: &mut Log) -> Result<(), String> {
    log.line(&format!("waiting for Glance (pid {pid}) to exit"));
    if !wait_for_exit(pid, Duration::from_secs(30)) {
        return Err("the old Glance did not exit within 30 seconds; nothing changed".into());
    }
    let exe = std::env::current_exe().map_err(|e| e.to_string())?;
    let from = exe.parent().ok_or("cannot find this binary's folder")?;
    let dir = if glance_hooks::dev() {
        from.to_path_buf()
    } else {
    ...
    if moved.is_empty() {
        return Err(
            "the new Glance did not come up, and there is no old build to go back to".into(),
        );
    }
    log.line("the new Glance did not come up, putting the old build back");
    restore(&dir, &moved);
    if start_and_check(&dir, log)? {
...
/// The Claude Code hooks, as `glance install` writes them, in case the new
/// build changed them. A failure is logged, not fatal: the old hooks still
/// reach Glance.
fn hooks(log: &mut Log) {
    let Some(settings) = glance_hooks::install::settings_path() else {
        return;
    };
    if let Err(e) = glance_hooks::install::install(&settings, glance_hooks::port()) {
        log.line(&format!("hooks not updated: {e}"));
    }
}

/// Starts `dir\glance.exe app --reload` and reports whether it came up and
...
/// `glance.exe` becomes `glance.old.exe`.
...
mod tests {
    ...
    fn the_old_binary_keeps_its_extension() {
        assert_eq!(old_name("glance.exe"), "glance.old.exe");
        assert_eq!(old_name("glancew.exe"), "glancew.old.exe");
        assert_eq!(old_name("glance"), "glance.old");
    }

    #[test]
    fn a_half_done_install_restores_only_what_it_moved() {
        let root = std::env::temp_dir().join(format!("glance-reload-{}", std::process::id()));
        let (from, dir) = (root.join("new"), root.join("installed"));
        fs::create_dir_all(&from).unwrap();
        fs::create_dir_all(&dir).unwrap();
        fs::write(from.join("glance.exe"), "new").unwrap();
        fs::write(dir.join("glance.exe"), "old").unwrap();
        fs::write(dir.join("glancew.exe"), "old w").unwrap();

        // glancew.exe is missing from the build, so nothing may move.
        let mut moved = Vec::new();
        assert!(put_in_place(&from, &dir, &mut moved).is_err());
        assert!(moved.is_empty());
        assert_eq!(fs::read_to_string(dir.join("glance.exe")).unwrap(), "old");

        fs::write(from.join("glancew.exe"), "new w").unwrap();
        put_in_place(&from, &dir, &mut moved).unwrap();
        assert_eq!(moved, BINARIES);
        assert_eq!(fs::read_to_string(dir.join("glance.exe")).unwrap(), "new");
        assert_eq!(
            fs::read_to_string(dir.join("glance.old.exe")).unwrap(),
            "old"
        );
        ...
        assert_eq!(fs::read_to_string(dir.join("glance.exe")).unwrap(), "old");
        assert_eq!(
            fs::read_to_string(dir.join("glancew.exe")).unwrap(),
            "old w"
        );
