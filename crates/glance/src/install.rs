//! `glance install`: makes Glance an app like any other.
//!
//! Copies both binaries to `%LOCALAPPDATA%\Programs\Glance`, where per user
//! apps live, then from there: puts the folder on the user's `PATH` so
//! `glance` works in any new terminal, adds a Start menu shortcut so Windows
//! search finds it, points Explorer's "Open in Glance" and Start with
//! Windows at the installed copy, and installs the Claude Code hooks. No
//! admin rights anywhere. `glance uninstall` takes all of it back out, and
//! leaves the saved sessions in `%APPDATA%\Glance`.
...
pub const BINARIES: [&str; 2] = ["glance.exe", "glancew.exe"];

/// Where the installed copy lives.
pub fn dir() -> Option<PathBuf> {
    std::env::var_os("LOCALAPPDATA").map(|d| PathBuf::from(d).join("Programs").join("Glance"))
}
...
pub fn install(running: bool) -> Result<PathBuf, String> {
    // A running Glance holds its executable open, so the copy would fail
    // half way.
    if running {
        return Err(
            "Glance is running. Quit it from the tray icon first, then install again.".into(),
        );
    }
...
pub fn uninstall(running: bool) -> Result<(), String> {
    if running {
        return Err(
            "Glance is running. Quit it from the tray icon first, then uninstall again.".into(),
        );
    }
    let dir = dir().ok_or("cannot find %LOCALAPPDATA%")?;
...
mod tests {
    ...
    fn adds_once_and_keeps_the_rest() {
        let dir = r"C:\Users\me\AppData\Local\Programs\Glance";
        assert_eq!(
    ...
    fn removes_only_its_own_entry() {
        let dir = r"C:\Glance";
        assert_eq!(
            path_without(r"C:\a;c:\glance\;C:\b", dir).unwrap(),
            r"C:\a;C:\b"
        );
