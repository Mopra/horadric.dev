...
//!
//! A Glance terminal is a real `claude` attached to a ConPTY: Windows runs
...
mod tests {
    ...
    fn environment_sets_removes_and_sorts() {
        let base = vec![
            ...
            ("glance_session".into(), "stale".into()),
            ("ALPHA".into(), "a".into()),
        ];
        let block = environment_block(
            base,
            &[("GLANCE_SESSION".into(), "fresh".into())],
            &["claudecode"],
        );
        let text = String::from_utf16(&block).unwrap();
        assert_eq!(text, "ALPHA=a\0GLANCE_SESSION=fresh\0Path=C:\\bin\0\0");
    }
