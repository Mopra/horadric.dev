//! What Claude Code says about usage, and the defaults Glance starts its
//! sessions with.
//!
...
//! session Glance starts gets `glance status` as its status line, which
...
/// A setting Glance passes to every `claude` it starts.
...
/// What every session Glance starts or resumes gets, unless its own
