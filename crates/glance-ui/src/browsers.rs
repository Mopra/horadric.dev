...
//! agent now runs in a job object (see `glance-pty`) and everything it
...
//! Glance started from inside one, whose windows are in that session's job
//! too.
//!
...
//! and a hung browser would freeze Glance.
...
/// `wparam`. Glance's own windows are skipped.
pub fn watch(notify: HWND, shown: u32, gone: u32) {
    ...
    if hook.is_invalid() {
        eprintln!("glance: cannot watch for browser windows");
    }
}
...
/// works right after real input to Glance, such as a click on a tile.
...
mod tests {
    ...
    fn browsers_are_known_by_their_executable() {
        ...
        assert!(!is_browser(
            r"C:\Users\x\AppData\Local\Programs\Glance\glance.exe"
        ));
