//! Reads and writes `%APPDATA%\Glance\state.json`, or `Glance-dev` for a
//! dev instance, which must never load or overwrite the real sessions.

use std::fs;
use std::path::{Path, PathBuf};

use glance_core::SavedState;
...
/// Writes the settings Glance hands each `claude` it starts with
/// `--settings`: `glance status` as the status line, run from `exe`. Only
    ...
    fn the_status_command_quotes_only_for_a_space() {
        assert_eq!(
            status_command(Path::new(
                r"C:\Users\me\AppData\Local\Programs\Glance\glance.exe"
            )),
            "C:/Users/me/AppData/Local/Programs/Glance/glance.exe status"
        );
        assert_eq!(
            status_command(Path::new(r"C:\Program Files\Glance\glance.exe")),
            "\"C:/Program Files/Glance/glance.exe\" status"
        );
    }
}
