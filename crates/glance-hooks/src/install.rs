//! Writes Glance's hook entries into Claude Code's user settings.
//!
...
//! started under Glance: the session header comes out empty and the listener
//! drops the event. The installer only ever touches entries whose URL
//! contains `/glance/`, so a user's own hooks are left exactly as they were.
...
/// Adds or refreshes Glance's hooks in a settings document.
...
/// Removes every Glance hook from a settings document, leaving the rest.
...
/// True when the document has a Glance hook on every event we need.
...
fn write(path: &Path, settings: &Value) -> io::Result<()> {
    ...
    let tmp = path.with_extension("json.glance-tmp");
