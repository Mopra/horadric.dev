//! A minimal HTTP/1.1 server for one purpose: accept `POST /glance/hook`.
//!
...
use crate::{HOOK_PATH, SESSION_HEADER};
...
pub fn serve(port: u16, tx: Sender<Tagged>) -> io::Result<()> {
...
fn handle(mut stream: TcpStream, tx: &Sender<Tagged>) -> io::Result<()> {
...
mod tests {
    ...
    fn start() -> (u16, mpsc::Receiver<Tagged>) {
        ...
        thread::spawn(move || serve(port, tx));
